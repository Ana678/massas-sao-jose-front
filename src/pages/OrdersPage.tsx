...
export default function OrdersPage() {
    ...
    const clientOptions = [{ value: "todos", label: "Todos os clientes" }, ...clients.map((client) => ({ value: client.id, label: client.name }))];
    const methodOptions = [
    ...
    return (
        <div className="pb-24">
                ...
                <Collapsible
                    trigger={
                        <div className="flex items-center gap-2 w-full">
                            ...
                            <span className="text-xs font-normal flex-1 text-left">Filtros detalhados</span>
                            <span className="text-muted-foreground text-xs">
                                {advancedFiltersCount > 0 ? `${advancedFiltersCount} ativo${advancedFiltersCount > 1 ? "s" : ""}` : "Período, cliente e pagamento"}
                            </span>
                        </div>
                    }
                >
                    <div className="bg-card rounded-xl border border-border p-3 space-y-3 mt-3">
                        ...
                        <SelectField
                            ...
                            value={methodFilter}
                            onChange={(value) => setMethodFilter(value as typeof methodFilter)}
            ...
            <section className="px-6 pb-6 space-y-2">
                {isLoading && (
                    <LoadingState message="Buscando histórico..." />
                )}
